    ...
    <Box
      ...
      <HStack>
        ...
        <Box ml={2}>
          ...
          <Text>{susuMember.member.role}</Text>
          <Text>Payment Position: {susuMember.position}</Text>
          <Text fontSize="md" fontFamily="body">Payment Date: {moment(susuMember.disbursementDate).format('YYYY, MMM DD')}</Text>
          {susuMember.isPaid ? (
            <Text fontSize="md" fontFamily="body">Paid on {moment(susuMember.disbursementDate).fromNow()}</Text>
          ) : (
